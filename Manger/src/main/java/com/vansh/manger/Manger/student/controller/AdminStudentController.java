...
public class AdminStudentController {
   ...
   public ResponseEntity<Page<StudentResponseDTO>> getAllStudents(
    ...
    @GetMapping("/{studentId:\\d+}/exam-results")
    public ResponseEntity<Page<StudentExamResultDTO>> getStudentExamResults(
            @PathVariable Long studentId,
            @PageableDefault(size = 10, sort = "id", direction = Sort.Direction.DESC) Pageable pageable) {
        return ResponseEntity.ok(adminStudentService.getStudentExamResults(studentId, pageable));
    }

    @GetMapping("/{studentId:\\d+}/attendance-summary")
    public ResponseEntity<AttendanceSummaryDTO> getStudentAttendanceSummary(@PathVariable Long studentId) {
        return ResponseEntity.ok(adminStudentService.getStudentAttendanceSummary(studentId));
    }
