...
import {
    ...
    Loader2, LogOut, MoreHorizontal, School, UserCircle, Activity
} from 'lucide-react';
...
const Sidebar = ({ isCollapsed, handleLogout, studentName, academicYear }) => {
    return (
        <aside className={`bg-slate-900 border-r border-slate-800/80 h-full flex flex-col transition-all duration-300 ease-in-out shrink-0 ${isCollapsed ? 'w-[4.5rem]' : 'w-64'} shadow-lg`}>
            ...
            <nav className="flex-grow p-3 overflow-y-auto no-scrollbar">
                <ul className="space-y-1">
                    <NavLink href="/student/dashboard" icon={LayoutDashboard} isCollapsed={isCollapsed}>Overview</NavLink>

                    {!isCollapsed && <p className="text-[10px] uppercase text-slate-500 font-bold mt-6 mb-2 px-3 tracking-wider">Academics</p>}
                    <NavLink href="/student/timetable" icon={CalendarClock} isCollapsed={isCollapsed}>My Timetable</NavLink>
                    <NavLink href="/student/attendance" icon={ClipboardCheck} isCollapsed={isCollapsed}>My Attendance</NavLink>

                    {!isCollapsed && <p className="text-[10px] uppercase text-slate-500 font-bold mt-6 mb-2 px-3 tracking-wider">Performance</p>}
                    <NavLink href="/student/exams" icon={GraduationCap} isCollapsed={isCollapsed}>Exam Results</NavLink>
                    <NavLink href="/student/risk-analysis" icon={Activity} isCollapsed={isCollapsed}>Risk Analysis</NavLink>
                </ul>
            </nav>
...
const DockItem = ({ href, icon: Icon, label }) => {
    ...
    const isActive = pathname === href || pathname.startsWith(`${href}/`);
    return (
        <Link href={href} className={`flex flex-col items-center justify-center gap-0.5 py-1.5 px-1 rounded-lg transition-colors min-w-0 flex-1 ${isActive ? 'text-orange-600' : 'text-slate-400 active:text-slate-600'}`}>
            <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
            <span className={`text-[10px] font-medium leading-tight truncate ${isActive ? 'text-orange-600' : 'text-slate-500'}`}>{label}</span>
        </Link>
    );
};
...
const SheetNavItem = ({ href, icon: Icon, label, onClose }) => {
    ...
    const isActive = pathname === href || pathname.startsWith(`${href}/`);
    return (
        <Link href={href} onClick={onClose} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${isActive ? 'bg-orange-50 text-orange-700 font-semibold' : 'text-slate-700 hover:bg-slate-100 active:bg-slate-200'}`}>
            <div className={`p-2 rounded-lg ${isActive ? 'bg-orange-100' : 'bg-slate-100'}`}>
                <Icon size={18} className={isActive ? 'text-orange-600' : 'text-slate-500'} />
            </div>
            <span className="text-sm font-medium">{label}</span>
            {isActive && <div className="ml-auto h-2 w-2 rounded-full bg-orange-600" />}
        </Link>
    );
};
    ...
    return (
        <StudentProtectedRoute>
            <div className="flex h-screen bg-slate-50 overflow-hidden">
                ...
                >
                    <Sidebar isCollapsed={!isSidebarHovered} handleLogout={handleLogout} studentName={studentName} academicYear={academicYear} />
                </div>
                ...
                <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden transition-all duration-300">
                            ...
                            <DropdownMenuContent align="end" className="w-56 mt-2">
                                <DropdownMenuLabel>
                                    <div className="flex flex-col">
                                        <span className="font-medium text-slate-900 truncate">{studentName}</span>
                                        <span className="text-xs text-slate-500 font-normal">{academicYear || 'Student'}</span>
                                    </div>
                                </DropdownMenuLabel>
                                <DropdownMenuSeparator />
                    ...
                    <main className="flex-1 overflow-y-auto pb-20 lg:pb-0 scroll-smooth">
                        <div className="student-page w-full min-h-full">
                            {children}
                        </div>
                    </main>
                </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
                    <div className="bg-white/95 backdrop-blur-lg border-t border-slate-200 shadow-[0_-2px_10px_rgba(0,0,0,0.06)]">
                        <div className="flex items-center justify-around px-2 py-1 max-w-lg mx-auto">
                            <DockItem href="/student/dashboard" icon={LayoutDashboard} label="Home" />
                            <DockItem href="/student/timetable" icon={CalendarClock} label="Timetable" />
                            <DockItem href="/student/attendance" icon={ClipboardCheck} label="Attendance" />
                            <DockItem href="/student/risk-analysis" icon={Activity} label="Risk" />
                            <button onClick={() => setIsMoreOpen(true)} className={`flex flex-col items-center justify-center gap-0.5 py-1.5 px-1 rounded-lg transition-colors min-w-0 flex-1 ${isMoreOpen ? 'text-orange-600' : 'text-slate-400 active:text-slate-600'}`}>
                ...
                <Sheet open={isMoreOpen} onOpenChange={setIsMoreOpen}>
                    <SheetContent side="bottom" className="rounded-t-3xl max-h-[80vh] px-4 pb-8 border-t border-slate-200">
                        ...
                        <div className="py-3 space-y-1 overflow-y-auto no-scrollbar">
                            <p className="text-[10px] uppercase text-slate-400 font-bold px-4 mb-2 tracking-wider">Performance</p>
                            <SheetNavItem href="/student/exams" icon={GraduationCap} label="Exam Results" onClose={() => setIsMoreOpen(false)} />

                            <p className="text-[10px] uppercase text-slate-400 font-bold px-4 mt-4 mb-2 tracking-wider">Account</p>
                            <SheetNavItem href="/student/profile" icon={UserCircle} label="My Profile" onClose={() => setIsMoreOpen(false)} />
